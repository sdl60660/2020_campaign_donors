 ...
 (SELECT donors_grouped.fec_id,
    ...
    FROM ( SELECT 
          ...
          WHERE contributions.candidate IS NOT NULL AND donors_with_expected_demographics.median_household_income IS NOT NULL AND 
          ((candidates.district_id <> donors_with_expected_demographics.house_district_id AND candidates.district_id <> donors_with_expected_demographics.senate_district_id)
            OR candidates.race_type = 'president')
 ...
 LEFT JOIN districts ON districts.id = candidates.district_id
    ) TO '/users/samlearner/miscellaneous_programming/portfolio_projects/candidate_contributions/data_processing/db_outputs/candidate_demographics_including_out_of_district.csv' DELIMITER ',' CSV HEADER;
